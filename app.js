(() => {
  ...
  function sidebar(active) {
    ...
    return `<aside class="sidebar">
      ...
      <div class="nav-group"><div class="nav-label">Workspace</div>
        ...
        ${nav('groups','그룹 관리','bi-people','/groups')}
  ...
  async function projectView(id){
    ...
    app.querySelectorAll('[data-provider]').forEach(b=>b.onclick=()=>{if(envObj.providerSource==='group'){navigate('/groups')}else openProviderModal(id,envObj,b.dataset.provider)}); app.querySelector('[data-open-account-group]')?.addEventListener('click',()=>navigate('/groups'));
  ...
  async function render(){
    ...
    await bootstrap();const route=location.hash.slice(1)||'/overview';state.route=route;if(route==='/overview')return overviewView();if(route==='/projects')return projectsView();if(route==='/groups')return window.AuthHubAccountGroups?.render({app,request,toast,loading,navigate,version:cfg.version,providers});if(route.startsWith('/project/'))return projectView(route.split('/')[2]);if(route==='/users')return usersView();if(route==='/audit')return auditView();if(route==='/integration')return integrationView();navigate('/overview')}
