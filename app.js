(() => {
  ...
  async function projectView(id){
    ...
    app.querySelectorAll('[data-provider]').forEach(b=>b.onclick=()=>openProviderModal(id,p,envObj,b.dataset.provider));
  ...
  async function openCreateProject(){
    ...
    let items=[]; let accountScopeGroups=[];
    try {
      const [data,groups]=await Promise.all([request('/admin/monitor/projects'),request('/admin/account-groups')]); items=data.items||[]; accountScopeGroups=(groups.items||[]).filter(g=>Number(g.project_count||0)>0);
    } catch(e) {
    ...
    let filtered=[];
    let activeIndex=-1;
    body.innerHTML=`<label class="field"><span>MONITOR 프로젝트</span><div class="monitor-combobox" id="m-monitor-combobox"><div class="monitor-combobox-input"><input class="input" id="m-monitor-project" role="combobox" aria-autocomplete="list" aria-expanded="false" aria-controls="m-monitor-options" placeholder="프로젝트명, pNN, URL 검색" autocomplete="off"><button type="button" class="monitor-combobox-toggle" tabindex="-1" aria-label="프로젝트 목록 열기"><i class="bi bi-chevron-down"></i></button></div><div class="monitor-combobox-menu" id="m-monitor-options" role="listbox"></div></div><small>이름·pNN·URL을 입력하면 같은 목록에서 바로 필터링됩니다. pNN은 MONITOR가 발급한 값을 그대로 사용합니다.</small></label><div id="m-monitor-selection" class="monitor-selection-line"></div><label class="field"><span>계정 범위</span><select class="select" id="m-account-group"><option value="">이 프로젝트만 사용 · 새 독립 범위</option>${accountScopeGroups.map(g=>`<option value="${esc(g.id)}">${esc((g.projects||[]).map(p=>p.name).join(' + ')||g.name)} · ${Number(g.project_count||0)}개 프로젝트</option>`).join('')}</select><small>기본값은 독립입니다. 기존 프로젝트와 같은 회원·소셜 키를 써야 할 때만 기존 범위를 선택하세요.</small></label><label class="field"><span>AuthHub 메모</span><textarea class="textarea" id="m-desc" placeholder="선택 사항"></textarea></label>`;
    ...
    save.onclick=async()=>{
      ...
      try{
        const accountGroupId=String(body.querySelector('#m-account-group')?.value||''); const r=await request('/admin/projects',{method:'POST',body:JSON.stringify({monitorProjectId:selected.project_id,description:body.querySelector('#m-desc').value,accountScope:accountGroupId?'existing':'own',accountGroupId})});
  ...
  function openProviderModal(projectId,projectObj,envObj,providerName){
    const p=envObj.providers.find(x=>x.provider===providerName)||{}; const meta=providers[providerName]||{}; const effectiveGroupId=envObj.providerSource==='group'?envObj.providerGroupId:''; const sharedCount=Math.max(1,Number(projectObj?.accountGroup?.project_count||projectObj?.accountGroup?.projects?.length||1));
    ...
    m.querySelector('[data-save]').onclick=async()=>{
      ...
      if(effectiveGroupId&&sharedCount>1&&!confirm(`이 변경은 현재 계정 범위를 공유하는 ${sharedCount}개 프로젝트의 ${meta.label||providerName} 로그인에 함께 적용됩니다. 저장할까요?`))return;
  ...
  async function render(){
    ...
    await bootstrap();const route=location.hash.slice(1)||'/overview';state.route=route;if(route==='/overview')return overviewView();if(route==='/projects')return projectsView();if(route==='/groups'){navigate('/projects');return}if(route.startsWith('/project/'))return projectView(route.split('/')[2]);if(route==='/users')return usersView();if(route==='/audit')return auditView();if(route==='/integration')return integrationView();navigate('/overview')}
