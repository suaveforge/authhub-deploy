...
if 'ACCOUNT_SCOPE_UI_V3' not in app:
    ...
    app,n=re.subn(pat,repl,app,count=1,flags=re.S)
    if n!=1: raise SystemExit(f'provider/scope line: expected 1 got {n}')

    sign='<section class="section"><div class="section-title"><div><h2>Sign-in methods</h2>'
    app=one(app,sign,'${scopeSection}'+sign,'insert scope section')
    app=app.replace('${groupProviderNotice}','',1)

    old_bind="    app.querySelectorAll('[data-provider]').forEach(b=>b.onclick=()=>{if(envObj.providerSource==='group'){navigate('/groups')}else openProviderModal(id,envObj,b.dataset.provider)}); app.querySelector('[data-open-account-group]')?.addEventListener('click',()=>navigate('/groups'));"
    new_bind="    app.querySelectorAll('[data-provider]').forEach(b=>b.onclick=()=>openProviderModal(id,p,envObj,b.dataset.provider));\n    app.querySelector('[data-account-scope]')?.addEventListener('click',()=>openAccountScopeModal(id,p));"
    app=one(app,old_bind,new_bind,'project bindings')

    app=one(app,'function openProviderModal(projectId,envObj,providerName){','function openProviderModal(projectId,projectObj,envObj,providerName){','provider signature')
    app=one(app,"    const p=envObj.providers.find(x=>x.provider===providerName)||{}; const meta=providers[providerName]||{};","    const p=envObj.providers.find(x=>x.provider===providerName)||{}; const meta=providers[providerName]||{}; const effectiveGroupId=envObj.providerSource==='group'?envObj.providerGroupId:''; const sharedCount=Math.max(1,Number(projectObj?.accountGroup?.project_count||projectObj?.accountGroup?.projects?.length||1));",'provider scope')
    old_save="      loading(true);try{await request(`/admin/environments/${envObj.id}/providers/${providerName}`,{method:'PUT',body:JSON.stringify(body)});m.remove();toast(`${meta.label||providerName} 설정을 저장했습니다.`);await projectView(projectId)}catch(e){toast(e.message,'error')}finally{loading(false)}"
    new_save="      if(effectiveGroupId&&sharedCount>1&&!confirm(`이 변경은 현재 계정 범위를 공유하는 ${sharedCount}개 프로젝트의 ${meta.label||providerName} 로그인에 함께 적용됩니다. 저장할까요?`))return;\n      const endpoint=effectiveGroupId?`/admin/account-groups/${encodeURIComponent(effectiveGroupId)}/providers/${encodeURIComponent(envObj.name)}/${providerName}`:`/admin/environments/${envObj.id}/providers/${providerName}`;\n      loading(true);try{await request(endpoint,{method:'PUT',body:JSON.stringify(body)});m.remove();toast(effectiveGroupId&&sharedCount>1?`${sharedCount}개 프로젝트의 ${meta.label||providerName} 공통 설정을 저장했습니다.`:`${meta.label||providerName} 설정을 저장했습니다.`);await projectView(projectId)}catch(e){toast(e.message,'error')}finally{loading(false)}"
    app=one(app,old_save,new_save,'provider save')
...
'''
    app=one(app,'\n  async function openBridgeModal(projectId){',scope_fn+'\n  async function openBridgeModal(projectId){','scope modal')

    app=one(app,"    let items=[];\n    try {\n      const data=await request('/admin/monitor/projects'); items=data.items||[];","    let items=[]; let accountScopeGroups=[];\n    try {\n      const [data,groups]=await Promise.all([request('/admin/monitor/projects'),request('/admin/account-groups')]); items=data.items||[]; accountScopeGroups=(groups.items||[]).filter(g=>Number(g.project_count||0)>0);",'create load scopes')
    anchor='</label><div id="m-monitor-selection" class="monitor-selection-line"></div><label class="field"><span>AuthHub 메모</span>'
    options="${accountScopeGroups.map(g=>`<option value=\"${esc(g.id)}\">${esc((g.projects||[]).map(p=>p.name).join(' + ')||g.name)} · ${Number(g.project_count||0)}개 프로젝트</option>`).join('')}"
    added='</label><div id="m-monitor-selection" class="monitor-selection-line"></div><label class="field"><span>계정 범위</span><select class="select" id="m-account-group"><option value="">이 프로젝트만 사용 · 새 독립 범위</option>'+options+'</select><small>기본값은 독립입니다. 기존 프로젝트와 같은 회원·소셜 키를 써야 할 때만 기존 범위를 선택하세요.</small></label><label class="field"><span>AuthHub 메모</span>'
    app=one(app,anchor,added,'create scope selector')
    app=one(app,"const r=await request('/admin/projects',{method:'POST',body:JSON.stringify({monitorProjectId:selected.project_id,description:body.querySelector('#m-desc').value})});","const accountGroupId=String(body.querySelector('#m-account-group')?.value||''); const r=await request('/admin/projects',{method:'POST',body:JSON.stringify({monitorProjectId:selected.project_id,description:body.querySelector('#m-desc').value,accountScope:accountGroupId?'existing':'own',accountGroupId})});",'create payload')
    app=one(app,"if(route==='/groups')return window.AuthHubAccountGroupsUI.render({app,state,request,toast,loading,navigate,version:cfg.version});","if(route==='/groups'){navigate('/projects');return}",'legacy route')
